...
\begin{dfn} \emph{Функцией Лагранжа} задачи \eqref{eq:cond-problem-problem-form}
  называют функцию
  \begin{equation*}
    \La(x, \lambda_0, \lambda) = \lambda_0 f(x) + \sum_{j=1}^m {\lambda_j g_j(x)}
  \end{equation*}
  где $\lambda$ — вектор $\lambda_1, \dotsc \lambda_m$.
\end{dfn}

\begin{thm}[Куна—Таккера]
  \label{th:kuhn-tucker}
  Пусть точка $x^*$ является решением задачи \eqref{eq:cond-problem-problem-form}
...
\begin{figure}[thb]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[very thick, red!50!black] coordinates{(0,5) (0,0) (1,0) (2,2)};
      \label{plot:boundaries}
    \end{axis}
  \end{tikzpicture}
  \caption{Ограничения \ref{plot:boundaries} и линии уровня целевой
    функции задачи \eqref{eq:cond-optim-problem}}
\end{figure}
