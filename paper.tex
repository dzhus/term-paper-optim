...
решения задачи \eqref{eq:cond-optim-problem-form} при линеаризации
входящих в неё функций. Поиск начинается в допустимой точке
пространства решений и продолжается по траектории, обеспечивающей
улучшение значения целевой функции, но не выходящей за границы
допустимой области.

Пусть в \eqref{eq:cond-optim-problem-form} целевая функция $f(x)$ и
функции ограничений $g_j(x)$ линеаризованы (с помощью разложения в ряд
...
\end{equation}
причём начальное приближение $x^0$ выбирается из числа точек,
...
\begin{equation*}
  \norm{f'(x^k)} < \epsilon
\end{equation*}
при удовлетворении которого процесс минимаизации прерывается.
