...
Далее рассмотривается ряд определений и теорем, которые используются в
решении, после чего формулируется точный алгоритм метода. Изложение
иллюстрируется практическим примером.

Более подробно данная тема освещена в \cite{alekseev05} и
\cite{taha05}.
...
\eqref{eq:cond-optim-problem-form}. Точки добавляются к решению (или,
наоборот, исключаются из дальнейшего рассмотрения) по мере выполнения
для них различных достаточных условий (невыполнения необходимых).
...
\item Добавить к решению точки условного минимума или максимума, если
  для них условия теоремы Куна—Таккера оказываются достаточными
  согласно замечанию \ref{rem:kt-cond}.
